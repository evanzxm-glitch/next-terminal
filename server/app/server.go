...
func setupRoutes() *echo.Echo {
	...
	handler := WrapHandler(http.StripPrefix(basePath, fileServer))
	routes := e.Group(basePath)
	routes.GET("", handler)
	routes.GET("/", handler)
	routes.GET("/branding", api.Branding)
	routes.GET("/favicon.ico", handler)
	routes.GET("/static/*", handler)
	...
	routes.Use(mw.ErrorHandler)
	routes.Use(mw.TcpWall)
	routes.Use(mw.Auth)
	//e.Use(RBAC)
	routes.Use(middleware.Gzip())
	...
	routes.POST("/login", accountApi.LoginEndpoint)
	...
	users := routes.Group("/users", mw.Admin)
	{
	...
	userGroups := routes.Group("/user-groups", mw.Admin)
	{
	...
	assets := routes.Group("/assets", mw.Admin)
	{
	...
	routes.GET("/tags", AssetApi.AssetTagsEndpoint)

	commands := routes.Group("/commands", mw.Admin)
	{
	...
	credentials := routes.Group("/credentials", mw.Admin)
	{
	...
	sessions := routes.Group("/sessions")
	{
	...
	loginLogs := routes.Group("/login-logs", mw.Admin)
	{
	...
	storageLogs := routes.Group("/storage-logs", mw.Admin)
	{
	...
	jobs := routes.Group("/jobs", mw.Admin)
	{
	...
	securities := routes.Group("/securities", mw.Admin)
	{
	...
	storages := routes.Group("/storages")
	{
	...
	strategies := routes.Group("/strategies", mw.Admin)
	{
	...
	accessGateways := routes.Group("/access-gateways", mw.Admin)
	{
	...
	roles := routes.Group("/roles", mw.Admin)
	{
	...
	loginPolicies := routes.Group("/login-policies", mw.Admin)
	{
	...
	authorised := routes.Group("/authorised", mw.Admin)
	{
	...
	routes.GET("/menus", RoleApi.TreeMenus, mw.Admin)
