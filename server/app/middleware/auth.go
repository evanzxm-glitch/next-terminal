...
func stripBasePath(uri string) string {
	if uri == nt.WebBasePath || uri == nt.WebBasePath+"/" {
		return "/"
	}
	if strings.HasPrefix(uri, nt.WebBasePath+"/") {
		return strings.TrimPrefix(uri, nt.WebBasePath)
	}
	return uri
}
...
func Auth(next echo.HandlerFunc) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		uri := stripBasePath(c.Request().URL.Path)
